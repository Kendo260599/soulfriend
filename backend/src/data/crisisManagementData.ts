...
export const crisisScenarios: CrisisScenario[] = [
  {
    ...
    level: 'critical',
    triggers: [
      'muốn chết', 'không muốn sống', 'kết thúc cuộc đời',
      'tự tử', 'nhảy lầu', 'uống thuốc', 'cắt tay',
      'không muốn tồn tại', 'muốn biến mất', 'không còn ý nghĩa'
    ],
    immediateResponse: 'Tôi rất quan tâm đến những gì bạn vừa chia sẻ. Những suy nghĩ này cho thấy bạn đang trải qua một giai đoạn rất khó khăn. Bạn không cần phải đối mặt một mình.',
    escalationProtocol: [
  ...
  },
  {
    ...
    level: 'high',
    triggers: [
      'cắt tay', 'đánh mình', 'làm đau bản thân',
      'tự hại', 'làm tổn thương', 'đánh vào tường',
      'không kiểm soát được', 'muốn đau đớn'
    ],
    immediateResponse: 'Tôi hiểu bạn đang trải qua rất nhiều đau đớn và có thể bạn đang tìm cách để giải tỏa. Có những cách an toàn hơn để xử lý những cảm xúc này.',
    escalationProtocol: [
  ...
  },
  {
    ...
    level: 'high',
    triggers: [
      'không thể ra khỏi giường', 'không ăn được', 'mất ngủ liên tục',
      'không có năng lượng', 'không thể làm việc', 'tuyệt vọng',
      'không thấy tương lai', 'cảm thấy vô dụng'
    ],
    immediateResponse: 'Tôi thấy bạn đang trải qua một giai đoạn rất khó khăn với những triệu chứng nghiêm trọng. Điều này có thể là dấu hiệu của trầm cảm nặng và cần được điều trị chuyên nghiệp.',
    escalationProtocol: [
  ...
  },
  {
    ...
    level: 'medium',
    triggers: [
      'không thở được', 'tim đập nhanh', 'hoảng loạn',
      'không kiểm soát được', 'sợ hãi tột độ', 'chóng mặt',
      'đổ mồ hôi', 'run rẩy'
    ],
    immediateResponse: 'Tôi hiểu bạn đang trải qua một cơn hoảng loạn và điều này có thể rất đáng sợ. Hãy thử một số kỹ thuật thở để giúp bạn bình tĩnh lại.',
    escalationProtocol: [
...
export const disclaimerTemplates: DisclaimerTemplate[] = [
  {
    ...
    context: 'general',
    template: 'Tôi là một chatbot hỗ trợ tâm lý và không thể thay thế cho việc tư vấn chuyên nghiệp. Nếu bạn đang gặp khủng hoảng nghiêm trọng, hãy liên hệ ngay với chuyên gia hoặc dịch vụ khẩn cấp.',
  ...
  },
  {
    ...
    context: 'crisis',
    template: 'Nếu bạn đang có ý định tự hại hoặc tự tử, hãy liên hệ ngay với đường dây nóng 1900 599 958 hoặc đến bệnh viện gần nhất. Tôi không thể thay thế cho việc chăm sóc y tế khẩn cấp.',
  ...
  },
  {
    ...
    template: 'Thông tin tôi cung cấp chỉ mang tính chất tham khảo và không thay thế cho chẩn đoán hoặc điều trị y tế chuyên nghiệp. Hãy tham khảo ý kiến bác sĩ hoặc chuyên gia tâm lý.',
    frequency: 'as_needed',
    placement: 'end'
  }
];
...
export function getRelevantReferral(
  ...
  if (urgency === 'critical') {
    filteredReferrals = filteredReferrals.filter(ref => 
      ref.type === 'hotline' || ref.type === 'hospital'
    );
  }
  ...
  if (userLocation) {
    filteredReferrals = filteredReferrals.filter(ref => 
      ref.location.includes(userLocation) || ref.location === 'Toàn quốc'
    );
  }
