...
export interface InteractionPattern {
  id: string;
  patternType: 'emotional_escalation' | 'topic_shift' | 'user_dissatisfaction' | 'successful_resolution';
...
export const interactionPatterns: InteractionPattern[] = [
  {
    ...
    triggerPhrases: [
      'không hiểu tôi', 'bạn không giúp được gì', 'tôi cần ai đó thật',
      'chatbot không thể hiểu', 'tôi muốn nói chuyện với người thật'
    ],
  ...
  },
  {
    ...
    triggerPhrases: [
      'cảm ơn bạn', 'điều này giúp tôi', 'tôi hiểu rồi',
      'bạn đã giúp tôi', 'tôi cảm thấy tốt hơn'
    ],
...
function evaluateRelevance(userInput: string, botResponse: string): number {
  ...
  const overlap = inputKeywords.filter(keyword => 
...
function evaluateEmpathy(botResponse: string): number {
  const empathyIndicators = [
    'tôi hiểu', 'tôi cảm thấy', 'tôi nghe thấy',
    'điều này khó khăn', 'bạn không cô đơn',
    'hoàn toàn bình thường', 'tôi ở đây'
  ];
...
function evaluateHelpfulness(botResponse: string): number {
  const helpfulnessIndicators = [
    'bạn có thể thử', 'tôi khuyên bạn',
    'hãy thử', 'có thể giúp',
    'resources', 'liên hệ', 'tìm kiếm'
  ];
...
function evaluateSafety(userInput: string, botResponse: string): number {
  ...
  if (hasCrisisTrigger) {
    const safetyIndicators = [
      'quan tâm', 'không cô đơn', 'liên hệ',
      'chuyên gia', 'khẩn cấp', 'hotline'
    ];
