...
const familyRelationshipQuestions: FamilyRelationshipQuestion[] = [
  // COMMUNICATION Domain (4 questions)
  {
    ...
    vietnameseText:
      'Các thành viên trong gia đình có thể nói chuyện cởi mở với nhau về mọi vấn đề.',
    englishText: 'Family members can talk openly with each other about any issue.',
    isReversed: false,
    culturalNote:
      'Open communication may be limited by hierarchical respect in Vietnamese families',
  },
  {
  ...
  // CONFLICT RESOLUTION Domain (4 questions)
  {
    ...
    vietnameseText:
      'Khi có xung đột, gia đình tôi có thể giải quyết một cách bình tĩnh và công bằng.',
  ...
  },
  {
    ...
    isReversed: false,
    culturalNote:
      'Traditional celebrations and ancestor worship are important in Vietnamese families',
  },
...
function calculateFamilyRelationshipScore(
  responses: FamilyRelationshipResponse[]
): FamilyRelationshipResult {
...
function identifyStrengthAreas(domainScores: any): string[] {
  ...
  Object.entries(domainScores).forEach(([domain, score]) => {
    if ((score as number) >= maxScore * 0.75) {
      // 75% or higher
      switch (domain) {
        case 'communication':
...
function identifyImprovementAreas(domainScores: any): string[] {
  ...
  Object.entries(domainScores).forEach(([domain, score]) => {
    if ((score as number) < maxScore * 0.5) {
      // Below 50%
      switch (domain) {
        case 'communication':
...
function generateFamilyRelationshipRecommendations(
  totalScore: number,
  domainScores: any
): string[] {
  ...
  if (totalScore >= 80) {
    recommendations.push(
      'Mối quan hệ gia đình của bạn rất xuất sắc. Hãy duy trì những điểm mạnh này.'
    );
...
function addDomainSpecificRecommendations(recommendations: string[], domainScores: any): void {
  ...
  if (domainScores.respect_trust < maxScore * 0.6) {
    recommendations.push(
      'Xây dựng lòng tin thông qua việc thực hiện lời hứa và tôn trọng ranh giới.'
    );
  }
}
...
function generateFamilyRelationshipCulturalConsiderations(domainScores: any): string[] {
  ...
  considerations.push(
    'Trong văn hóa Việt Nam, hòa hợp gia đình được đề cao và xung đột thường được tránh.'
  );

  if (domainScores.communication < 15) {
    considerations.push(
      'Giao tiếp có thể bị ảnh hưởng bởi thứ bậc trong gia đình - cân bằng tôn trọng và cởi mở.'
    );
  }

  if (domainScores.conflict_resolution < 15) {
    considerations.push(
      'Việc tránh xung đột để giữ "thể diện" có thể cản trở việc giải quyết vấn đề.'
    );
  }
...
const familyRelationshipTest = {
  testInfo: {
    ...
    description:
      'Đánh giá chất lượng mối quan hệ trong gia đình qua 5 lĩnh vực: Giao tiếp, Giải quyết xung đột, Hỗ trợ cảm xúc, Hoạt động chung, Tôn trọng & Tin tưởng',
    version: '2.0',
