...
export const userSegments: UserSegment[] = [
  {
    ...
    description: 'Tập trung vào các vấn đề thay đổi nội tiết tố, trầm cảm sau sinh và áp lực làm mẹ',
    keywords: [
      'mang thai', 'sau sinh', 'nội tiết tố', 'hormone', 'trầm cảm sau sinh', 'PPD',
      'áp lực làm mẹ', 'cho con bú', 'mất ngủ', 'kiệt sức', 'lo lắng về con',
      'thay đổi cơ thể', 'tăng cân', 'giảm cân', 'da xấu', 'tóc rụng',
      'cô đơn', 'không ai hiểu', 'chồng không quan tâm', 'gia đình chồng'
    ],
    emotionalPatterns: [
      'kiệt sức', 'vỡ mộng', 'bị thao túng', 'bị phớt lờ', 'tội lỗi',
      'lo lắng quá mức', 'hoảng sợ', 'tuyệt vọng', 'cô đơn', 'bị bỏ rơi'
    ],
    commonIssues: [
      'Trầm cảm sau sinh',
      'Áp lực cho con bú',
    ...
    responseTemplates: [
      {
        ...
        response: 'Tôi hiểu bạn đang trải qua giai đoạn rất khó khăn. Việc chăm sóc em bé là công việc 24/7 và hoàn toàn bình thường khi bạn cảm thấy kiệt sức. Bạn có thể chia sẻ thêm về tình hình hiện tại không?',
        empathyLevel: 'high',
        urgencyLevel: 'medium'
      },
      {
        trigger: ['trầm cảm sau sinh', 'PPD', 'buồn không muốn sống'],
        response: 'Tôi rất quan tâm đến những gì bạn đang chia sẻ. Trầm cảm sau sinh là tình trạng nghiêm trọng và bạn không cần phải đối mặt một mình. Tôi khuyên bạn nên liên hệ ngay với bác sĩ hoặc chuyên gia tâm lý.',
        empathyLevel: 'high',
  ...
  },
  {
    ...
    keywords: [
      'độc thân', 'sự nghiệp', 'công việc', 'áp lực xã hội', 'cô đơn',
      'kết hôn muộn', 'sinh con muộn', 'đồng hồ sinh học', 'biological clock',
      'cân bằng công việc', 'burnout', 'stress công việc', 'cạnh tranh',
      'glass ceiling', 'bình đẳng giới', 'lương thấp', 'thăng tiến'
    ],
    emotionalPatterns: [
      'cô đơn', 'lo lắng về tương lai', 'áp lực từ xã hội', 'tự ti',
      'kiệt sức', 'vỡ mộng', 'bị đánh giá', 'không được công nhận'
    ],
    commonIssues: [
    ...
    responseTemplates: [
      {
        ...
        response: 'Cảm giác cô đơn là điều rất phổ biến, đặc biệt khi bạn đang tập trung vào sự nghiệp. Điều quan trọng là bạn đang xây dựng một cuộc sống có ý nghĩa cho chính mình. Bạn có thể chia sẻ thêm về những gì khiến bạn cảm thấy cô đơn không?',
        empathyLevel: 'high',
        urgencyLevel: 'medium'
      },
      {
        trigger: ['áp lực kết hôn', 'gia đình thúc ép', 'tuổi tác'],
        response: 'Áp lực từ gia đình về việc kết hôn có thể rất khó chịu. Bạn có quyền sống theo cách mình muốn và tìm kiếm hạnh phúc theo cách riêng. Hãy nhớ rằng bạn không cần phải vội vàng để làm hài lòng người khác.',
        empathyLevel: 'high',
        urgencyLevel: 'low'
      }
    ],
  ...
  },
  {
    ...
    keywords: [
      'mãn kinh', 'tiền mãn kinh', 'perimenopause', 'menopause',
      'thay đổi cơ thể', 'bốc hỏa', 'hot flash', 'đổ mồ hôi đêm',
      'mất ngủ', 'thay đổi tâm trạng', 'lo âu', 'trầm cảm',
      'giảm ham muốn', 'khô âm đạo', 'tăng cân', 'da khô',
      'tóc mỏng', 'loãng xương', 'tim mạch', 'tuổi già'
    ],
    emotionalPatterns: [
      'lo lắng về tuổi già', 'tự ti về ngoại hình', 'buồn bã',
      'cáu kỉnh', 'kiệt sức', 'vỡ mộng', 'sợ hãi về tương lai'
    ],
    commonIssues: [
    ...
    responseTemplates: [
      {
        ...
        response: 'Bốc hỏa là triệu chứng rất phổ biến trong giai đoạn mãn kinh và có thể rất khó chịu. Đây là phản ứng bình thường của cơ thể khi nội tiết tố thay đổi. Bạn có thể thử một số cách để giảm bớt như mặc quần áo thoáng mát, tránh thức ăn cay, và tập thể dục nhẹ nhàng.',
        empathyLevel: 'high',
        urgencyLevel: 'low'
      },
      {
        trigger: ['lo lắng về tuổi già', 'sợ hãi', 'tương lai'],
        response: 'Lo lắng về tuổi già là cảm xúc rất tự nhiên. Giai đoạn mãn kinh đánh dấu một chương mới trong cuộc đời với những thay đổi và cơ hội mới. Bạn có thể chia sẻ thêm về những gì khiến bạn lo lắng không?',
        empathyLevel: 'high',
        urgencyLevel: 'medium'
      }
    ],
...
export function getResponseTemplate(
  ...
  for (const template of segment.responseTemplates) {
    const hasTrigger = template.trigger.some(trigger => 
...
export function analyzeNuancedEmotion(userInput: string): {
  ...
  const emotionMap: Record<string, { emotion: string; intensity: 'low' | 'medium' | 'high' | 'critical' }> = {
