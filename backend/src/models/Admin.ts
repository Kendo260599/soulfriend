...
// Tạo index (chỉ cho các fields không có unique: true)
...
AdminSchema.methods.incrementLoginAttempts = async function (): Promise<void> {
  ...
  // Tăng loginAttempts và kiểm tra lock
  const result = await this.updateOne({
    $inc: { loginAttempts: 1 },
  });

  // Sau khi update, kiểm tra xem có cần lock không
  const updatedAdmin = await (this.constructor as any).findById(this._id);
  if (updatedAdmin && updatedAdmin.loginAttempts >= 5 && !updatedAdmin.lockUntil) {
