 ...
 */
router.post('/submit', [
  body('testType').isIn(['DASS-21', 'GAD-7', 'PHQ-9', 'EPDS', 'SELF_COMPASSION', 'MINDFULNESS', 'SELF_CONFIDENCE', 'ROSENBERG_SELF_ESTEEM', 'PMS', 'MENOPAUSE_RATING'])
...
function calculateGADEvaluation(totalScore: number) {
  if (totalScore <= 4) {
    return { 
      level: 'minimal', 
      description: 'Lo âu ở mức tối thiểu. Bạn đang có trạng thái tâm lý khá ổn định và ít có dấu hiệu lo âu đáng lo ngại.' 
    };
  }
  if (totalScore <= 9) {
    return { 
      level: 'mild', 
      description: 'Lo âu ở mức nhẹ. Bạn có thể thỉnh thoảng cảm thấy lo lắng nhưng vẫn có thể kiểm soát và thực hiện các hoạt động hàng ngày.' 
    };
  }
  if (totalScore <= 14) {
    return { 
      level: 'moderate', 
      description: 'Lo âu ở mức vừa phải. Bạn có thể cảm thấy lo lắng thường xuyên hơn và điều này có thể ảnh hưởng đến một số hoạt động trong cuộc sống.' 
    };
  }
  return { 
    level: 'severe', 
    description: 'Lo âu ở mức cao. Bạn có thể đang trải qua lo âu nghiêm trọng ảnh hưởng đáng kể đến cuộc sống hàng ngày. Nên tìm kiếm sự hỗ trợ chuyên môn.' 
  };
}
...
function calculatePHQEvaluation(totalScore: number) {
  if (totalScore <= 4) {
    return { 
      level: 'minimal', 
      description: 'Trầm cảm ở mức tối thiểu. Bạn có tâm trạng khá ổn định và ít có dấu hiệu trầm cảm đáng lo ngại.' 
    };
  }
  if (totalScore <= 9) {
    return { 
      level: 'mild', 
      description: 'Trầm cảm ở mức nhẹ. Bạn có thể thỉnh thoảng cảm thấy buồn bã nhưng vẫn có thể thực hiện các hoạt động hàng ngày bình thường.' 
    };
  }
  if (totalScore <= 14) {
    return { 
      level: 'moderate', 
      description: 'Trầm cảm ở mức vừa phải. Bạn có thể cảm thấy buồn bã thường xuyên hơn và điều này có thể ảnh hưởng đến một số hoạt động trong cuộc sống.' 
    };
  }
  if (totalScore <= 19) {
    return { 
      level: 'moderately_severe', 
      description: 'Trầm cảm ở mức khá nặng. Bạn có thể đang trải qua những triệu chứng trầm cảm đáng kể ảnh hưởng đến cuộc sống hàng ngày. Nên tìm kiếm sự hỗ trợ chuyên môn.' 
    };
  }
  return { 
    level: 'severe', 
    description: 'Trầm cảm ở mức nặng. Bạn có thể đang trải qua trầm cảm nghiêm trọng ảnh hưởng rất nhiều đến cuộc sống. Hãy tìm kiếm sự hỗ trợ từ chuyên gia tâm lý ngay lập tức.' 
  };
}
...
function calculateEPDSEvaluation(totalScore: number) {
  if (totalScore < 10) {
    return { 
      level: 'low', 
      description: 'Ít khả năng trầm cảm sau sinh. Bạn đang có tâm trạng khá ổn định sau khi sinh con.' 
    };
  }
  if (totalScore <= 12) {
    return { 
      level: 'moderate', 
      description: 'Nguy cơ trầm cảm sau sinh ở mức vừa phải. Nên theo dõi tâm trạng và tìm kiếm sự hỗ trợ từ gia đình hoặc chuyên gia.' 
    };
  }
  return { 
    level: 'high', 
    description: 'Nguy cơ trầm cảm sau sinh cao. Hãy tìm kiếm sự hỗ trợ từ bác sĩ hoặc chuyên gia tâm lý ngay lập tức để được đánh giá và điều trị phù hợp.' 
  };
}
...
function calculateSelfCompassionEvaluation(totalScore: number) {
  ...
  if (totalScore <= 24) {
    return { 
      level: 'low', 
      description: 'Mức độ tự yêu thương thấp. Bạn có xu hướng khắt khe với bản thân. Hãy thực hành các kỹ thuật tự yêu thương để cải thiện.' 
    };
  }
  if (totalScore <= 42) {
    return { 
      level: 'moderate', 
      description: 'Mức độ tự yêu thương vừa phải. Bạn có thể thỉnh thoảng yêu thương bản thân nhưng vẫn còn chỗ để phát triển thêm.' 
    };
  }
  return { 
    level: 'high', 
    description: 'Mức độ tự yêu thương cao. Bạn biết cách đối xử tử tế và yêu thương với bản thân khi gặp khó khăn.' 
  };
}
...
function calculateMindfulnessEvaluation(totalScore: number) {
  ...
  if (totalScore <= 45) {
    return { 
      level: 'low', 
      description: 'Mức độ chánh niệm thấp. Bạn có xu hướng sống trong quá khứ hoặc tương lai nhiều hơn hiện tại. Thực hành thiền định và chánh niệm sẽ có ích.' 
    };
  }
  if (totalScore <= 67) {
    return { 
      level: 'moderate', 
      description: 'Mức độ chánh niệm vừa phải. Bạn đôi khi có thể tập trung vào hiện tại nhưng vẫn dễ bị phân tâm.' 
    };
  }
  return { 
    level: 'high', 
    description: 'Mức độ chánh niệm cao. Bạn có khả năng tốt trong việc sống trong hiện tại và nhận thức về những gì đang diễn ra xung quanh.' 
  };
}
...
function calculateSelfConfidenceEvaluation(totalScore: number) {
  ...
  if (totalScore <= 35) {
    return { 
      level: 'low', 
      description: 'Mức độ tự tin thấp. Bạn có thể đang thiếu niềm tin vào khả năng của bản thân. Hãy tập trung vào những thành công nhỏ và phát triển các kỹ năng tự tin.' 
    };
  }
  if (totalScore <= 70) {
    return { 
      level: 'moderate', 
      description: 'Mức độ tự tin vừa phải. Bạn có tự tin trong một số lĩnh vực nhưng vẫn có thể cải thiện thêm ở những khía cạnh khác.' 
    };
  }
  return { 
    level: 'high', 
    description: 'Mức độ tự tin cao. Bạn có niềm tin mạnh mẽ vào khả năng của bản thân trong hầu hết các lĩnh vực của cuộc sống.' 
  };
}
...
function calculateRosenbergEvaluation(totalScore: number) {
  if (totalScore <= 15) {
    return { 
      level: 'low', 
      description: 'Lòng tự trọng thấp. Bạn có thể thường xuyên đánh giá tiêu cực về bản thân. Hãy thực hành các kỹ thuật xây dựng lòng tự trọng tích cực.' 
    };
  }
  if (totalScore <= 25) {
    return { 
      level: 'moderate', 
      description: 'Lòng tự trọng ở mức bình thường. Bạn có cái nhìn cân bằng về bản thân với cả những điểm mạnh và điểm yếu.' 
    };
  }
  if (totalScore <= 30) {
    return { 
  ...
  return { 
    level: 'very_high', 
    description: 'Lòng tự trọng rất cao. Bạn có niềm tin mạnh mẽ vào giá trị và khả năng của bản thân.' 
  };
}
...
router.get('/health-check', async (req: Request, res: Response) => {
  try {
    ...
    const overallHealth = Object.values(healthStatus).every(status => 
      status === 'connected' || status === 'operational' || status === 'ready' || 
      status === 'active' || status === 'available' || typeof status === 'string'
    );
