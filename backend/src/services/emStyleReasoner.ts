    ...
    private findSimilarExamples(userMessage: string, count: number = 3): TrainingSample[] {
        ...
        const scored = this.trainingSamples.map(sample => {
            ...
            const commonWords = messageWords.filter(w => 
                sampleWords.includes(w) && w.length > 2
            );
...
⚠️ AN TOÀN: KHÔNG đưa lời khuyên y khoa/pháp lý. Chỉ self-help an toàn.`;
    ...
    async reason(
        ...
        try {
            ...
            if (crisisKeywords.some(kw => lowerMessage.includes(kw))) {
                return {
                    message: '⚠️ Tôi phát hiện dấu hiệu khủng hoảng. Đây là tình huống cần chuyên gia ngay lập tức.\n\n' +
            ...
            if (openAIService.isReady()) {
                try {
                    ...
                    if (response.confidence < 0.5) {
                        logger.warn('AI returned low confidence (likely error), using offline training service');
                        ...
                        logger.info(`📚 Offline training (confidence: ${offlineResponse.confidence.toFixed(2)}, source: ${offlineResponse.source})`);
                    } else {
                ...
                logger.info(`📚 Using offline training service (confidence: ${offlineResponse.confidence.toFixed(2)}, source: ${offlineResponse.source}, matches: ${offlineResponse.matchedSamples})`);
            }
    ...
    private parseResponse(responseText: string, originalMessage: string): EMResponse {
        ...
        const constraints = constraintsMatch
            ? constraintsMatch[1].split(/[,;]/).map(s => s.trim()).filter(Boolean)
            : ['10 phút/ngày', 'Self-help an toàn'];

        const keyVariables = variablesMatch
            ? variablesMatch[1].split(/[,;]/).map(s => s.trim()).filter(Boolean)
...
⚠️ Đây là mô phỏng phong cách tư duy, không thay thế chuyên gia.`;
    ...
    private inferGoal(message: string): string {
        const lower = message.toLowerCase();
        if (lower.includes('kiệt sức')) return 'Giảm kiệt sức trong 14 ngày';
        if (lower.includes('lo âu')) return 'Giảm lo âu trong 4 tuần';
        if (lower.includes('ngủ')) return 'Ngủ ngon hơn trong 2 tuần';
        return 'Cải thiện tình trạng trong 2-4 tuần';
    }

    private inferVariables(message: string): string[] {
        const lower = message.toLowerCase();
        if (lower.includes('ngủ')) return ['Giờ ngủ', 'Caffeine', 'Thời gian màn hình'];
        if (lower.includes('công việc')) return ['Tải công việc', 'Hỗ trợ xã hội'];
