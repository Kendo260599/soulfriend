    ...
    private loadTrainingData(): void {
        try {
            ...
            if (samplesPath) {
                ...
                logger.info(`✅ Loaded ${this.trainingSamples.length} training samples for offline learning`);
            } else {
...
⚠️ Đây là mô phỏng phong cách tư duy, không thay thế chuyên gia.`;
     ...
     */
    recordFeedback(userMessage: string, response: string, wasHelpful: boolean, quality?: number): void {
        ...
        if (matches.length > 0) {
            ...
            logger.info(`📊 Updated quality score for topic "${topic}": ${currentQuality.toFixed(1)} → ${newQuality.toFixed(1)}`);
        }
    }
    ...
    private isStopWord(word: string): boolean {
        const stopWords = [
            'và', 'của', 'cho', 'với', 'từ', 'đến', 'trong', 'này', 'đó',
            'mình', 'em', 'tôi', 'bạn', 'của', 'là', 'có', 'được', 'sẽ',
            'vì', 'để', 'nhưng', 'nếu', 'khi', 'thì', 'mà', 'nên',
        ];
    ...
    getStats(): {
        ...
        const avgQuality = this.trainingSamples.reduce((sum, s) => sum + (s.metadata.quality || 5), 0) / this.trainingSamples.length;
