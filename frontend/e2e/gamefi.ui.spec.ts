...
test('GameFi full click-through on browser UI', async ({ page }) => {
  ...
  await journalBox.fill('Hôm nay mình bình tĩnh hơn.\nMình biết ơn gia đình.\nMình muốn tiếp tục tiến bộ.');
  ...
  await journalBox.fill('Mình đã hiểu cảm xúc tốt hơn.\nMình đã dừng lại để thở.\nMình thấy nhẹ hơn.');
  await page.getByRole('button', { name: '✨ Gửi & Hoàn thành' }).click();
  await page.getByRole('button', { name: 'Đóng' }).click();

  const weeklyCard = page.getByText('1 cuộc trò chuyện sâu').first();
  await Promise.all([weeklyCard.click(), weeklyCard.click(), weeklyCard.click()]);
  await page.getByRole('button', { name: 'Đóng' }).click();
  await expect.poll(() => callCounters.weekly).toBe(1);
  ...
  await expect(page.getByText('🔒 Cần Level 8 và Growth 80 để mở khóa')).toBeVisible();

  await page.getByRole('button', { name: '🌳 Skill Tree' }).click();
  await expect(page.getByText('🌳 Skill Tree — Cây Kỹ Năng Tâm Lý')).toBeVisible();

  await page.getByRole('button', { name: '📜 Lore' }).click();
  await expect(page.getByText('📜 Vùng Đất Nội Tâm')).toBeVisible();
